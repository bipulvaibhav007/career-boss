                    ...
                    <div class="card">
                        <div class="card-header">
                          <ul class="nav nav-pills">
                            ...
                            <li class="nav-item">
                              <a class="nav-link active" href="<?=base_url('account-quiz')?>">Quiz</a>
                            </li>
                            
                          </ul>
                        </div>
                        <div class="card-body">
                          <div class="h5">Your quiz count (<?=$totQuiz ?>)</div>
                          <?php if(!empty($quiz_score)){
                            echo '<div class="row">';
                            foreach($quiz_score as $list){ 
                            $grade = 'N/A';
                            if($list->grade == 'Excellent'){
                              $grade = '<span class="text-success">E (Excellent)</span>';
                            }elseif($list->grade == 'Best'){
                              $grade = '<span class="text-info">BT (Best)</span>';
                            }elseif($list->grade == 'Better'){
                              $grade = '<span class="text-primary">BTR (Better)</span>';
                            }elseif($list->grade == 'Good'){
                              $grade = '<span class="text-warning">G (Good)</span>';
                            }elseif($list->grade == 'Fails'){
                              $grade = '<span class="text-danger">F (Fail)</span>';
                            }
                            ?>
                              <div class="col-md-6 my-2">
                                <div class="card">
                                  <table class="table">
                                    <tr>
                                      <td>Subject: <?=$list->module_name?><br>Date: <?=date('M d, Y',strtotime($list->createdAt))?></td>
                                      <td colspan="2">Grade: <?=$grade?></td>
                                    </tr>
                                    <tr>
                                      <td><?=$list->total_question.'<br>Total Question'?></td>
                                      <td class="text-success"><?=$list->correct.'<br>Correct'?></td>
                                      <td class="text-danger"><?=$list->wrong.'<br>Wrong'?></td>
                                    </tr>
