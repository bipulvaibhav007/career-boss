              ...
              <tbody>
                <?php if(!empty($student_list)){
                    ...
                    <tr <?=($list->status=='3')?'style="color:red;"':''?>>
                        ...
                        <td><?=$status?></td>
                        <td>
                            <?php if(is_privilege(32,2)){ 
                              if($list->course_cat == 'C' || $list->course_cat == 'T'){ ?>
                            <a href="<?= base_url('/institute/marksheet_cu/'.$list->id) ?>" class="btn btn-primary btn-sm" role="button" title="Marksheet"><?=($list->module_marks != '')?'Edit':'Add'?> Marks</a>

                            <?php } } if(is_privilege(32,3) ){ 
                              if((($list->course_cat == 'C' || $list->course_cat == 'T') && $list->module_marks != '') || ($list->course_cat == 'P' || $list->course_cat == 'F')){?>
                            <a href="javascript:void(0)" class="mt-2 btn btn-primary btn-sm Generate_Cert" data-id="<?=$list->id ?>" role="button" title="Generate Certificate" >Generate Certificate</a>
                            <?php }elseif($list->course_cat == 'U'){ ?>
                              <a href="<?=base_url('institute/universityinfo_cu/'.$list->id)?>" class="btn btn-info btn-sm" >Update University</a>
                            <?php } } ?>
                            <a href="javascript:void(0)" class="mt-2 btn btn-warning btn-sm Change_Status" data-id="<?=$list->id ?>" role="button" title="Change Status" >Change Status</a>
...
<script>
    ...
    $(".Generate_Cert").click(function(){
      if(confirm('Are u sure want to generate certificate?')){
        var url = "<?=base_url('institute/certified_students')?>";
