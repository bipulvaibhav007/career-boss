...
<div class="container-fluid" id="container-wrapper">
  <div class="d-sm-flex align-items-center justify-content-between mb-1">
    <h1 class="h3 mb-0 text-gray-800">Schedule View</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="<?=base_url('admin'); ?>">Admin</a></li>
      <li class="breadcrumb-item" aria-current="page">view_schedule</li>
      ...
      <div class="card mb-4">
        <div class="card-header py-1 bg-dark d-flex justify-content-between text-light">
          ...
          <div class="">
            ...
            <a href="<?=base_url('admin/exam_schedule')?>" class="btn btn-danger btn-sm">Back</a>
        ...
        <div class="card-body">
          <table class="table table-bordered">
            ...
            */ ?>
            <tr>
              <th>Exam Name</th>
              <td><?=strtoupper($sch->exam_name)?></td>
              <th>Courses</th>
              <?php $adminmodel = model('App\Models\Admin_model', false);
              $courses_name = $adminmodel->get_courses_name($sch->course_ids); ?>
              <td><?=$courses_name?></td>
            </tr>
            ...
            <tr>
                <th>Time From</th>
                <td><?=date('h:i:s A',strtotime($sch->time_from))?></td>
                <th>Time To</th>
                <td><?=date('h:i:s A',strtotime($sch->time_to))?></td>
      ...
      <div class="card mb-2">
        ...
        <div class="card-body">
          <table class="table table-bordered table-striped table-responsive">
            ...
            <tbody>
              <?php 
              $student_list = $adminmodel->get_examinee_list($sch->id);
              if(!empty($student_list)){ 
                ...
                ?>
                  <tr>
                    ...
                    <td><?php if($list->ex_status == 2){ echo '<a href="javascript:void(0)" onclick="view_examination_result('.$list->id.')">'.$list->frstu_name.'</a></td>';}else{ echo $list->frstu_name; } ?>
                    <td><?=$list->so_wo_do?></td>
                    ...
                    <td>
                        <?php 
                          $status = get_examinee_status_txt($list->ex_status);
                        
                          echo $status; ?>
                    </td>
                    
                    <td>
                        <?php if($list->ex_status == 2){ ?>
                        <a href="javascript:void(0);" class="btn btn-outline-info btn-sm " onclick="view_examination_result(<?=$list->id?>)"><i class="fas fa-eye"></i></a>
                        <?php } ?>
                        <?php /*
  ...
  */ ?>
  ...
  <div class="modal fade" id="franchiseStudentModal" tabindex="-1" role="dialog" aria-labelledby="franchiseStudentModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" >
      <div class="modal-content">
        <div class="modal-header bg-success">
          <h5 class="modal-title text-light" id="franchiseStudentModalLabel">Student Result</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
  ...
  <script>
    function view_examination_result(id){
      if(id){
        $.ajax({
          type: "post",
          url: "<?=base_url('admin/view_student_result_by_ajax');?>",
          data: {id:id},
          beforeSend: function() {
            // loader open
            $("#loader").show();
